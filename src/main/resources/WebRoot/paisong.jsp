...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"paisong",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update dingdan set zhuangtai='已派送' where id='"+request.getParameter("dingdanid")+"'");
    ...
    String charuid = new CommDAO().update(request,"paisong",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("paisong").find(Request.get("id"));
    
    new CommDAO().delete(request,"paisong");  // 执行删除命令
