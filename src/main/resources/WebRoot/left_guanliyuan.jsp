<%@ page language="java" import="dao.*" pageEncoding="UTF-8" %>
<%@ page language="java" import="java.util.*" %>
<%@page import="util.*" %>


<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>管理员账号管理</span>
    </a>
    <ul class="sub">
        <li><a href="admins_add.jsp" target="main">管理员账号添加</a></li>
        <li><a href="admins_list.jsp" target="main">管理员账号管理</a></li>
        <li><a href="mod.jsp" target="main">密码修改</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>骑士管理</span>
    </a>
    <ul class="sub">
        <li><a href="qishi_add.jsp" target="main">骑士添加</a></li>
        <li><a href="qishi_list.jsp" target="main">骑士查询</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>用户管理</span>
    </a>
    <ul class="sub">
        <li><a href="yonghu_add.jsp" target="main">用户添加</a></li>
        <li><a href="yonghu_list.jsp" target="main">用户查询</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>商品管理</span>
    </a>
    <ul class="sub">
        <li><a href="shangpin_add.jsp" target="main">商品添加</a></li>
        <li><a href="shangpin_list.jsp" target="main">商品查询</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>商品分类管理</span>
    </a>
    <ul class="sub">
        <li><a href="shangpinfenlei_add.jsp" target="main">商品分类添加</a></li>
        <li><a href="shangpinfenlei_list.jsp" target="main">商品分类查询</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>订单管理</span>
    </a>
    <ul class="sub">
        <li><a href="dingdan_list.jsp" target="main">订单查询</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>派送管理</span>
    </a>
    <ul class="sub">
        <li><a href="paisong_list.jsp" target="main">派送查询</a></li>
    </ul>
</li>
<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>签收管理</span>
    </a>
    <ul class="sub">
        <li><a href="qianshou_list.jsp" target="main">签收查询</a></li>
    </ul>
</li>

<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>评论管理</span>
    </a>
    <ul class="sub">
        <li><a href="pinglun_list.jsp" target="main">评论查询</a></li>
    </ul>
</li>

<li class="sub-menu">
    <a href="javascript:;">
        <i class="fa fa-angle-right"></i>
        <span>系统管理</span>
    </a>
    <ul class="sub">
        <li><a href="lunbotu_add.jsp" target="main">轮播图添加</a></li>
        <li><a href="lunbotu_list.jsp" target="main">轮播图查询</a></li>
        <li><a href="youqinglianjie_add.jsp" target="main">友情链接添加</a></li>
        <li><a href="youqinglianjie_list.jsp" target="main">友情链接查询</a></li>
    </ul>
</li>
