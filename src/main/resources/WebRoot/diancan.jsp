...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"diancan",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("UPDATE diancan SET xiaoji=(goumaishuliang*jiage+songcanfei) WHERE id='"+charuid+"'");
    ...
    String charuid = new CommDAO().update(request,"diancan",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法

    new CommDAO().commOper("UPDATE diancan SET xiaoji=(goumaishuliang*jiage+songcanfei) WHERE id='"+request.getParameter("id")+"'");
    ...
    // 获取一行数据
    HashMap map = Query.make("diancan").find(Request.get("id"));
    
    new CommDAO().delete(request,"diancan");  // 执行删除命令
