...
if(a.equals("insert")){
    // 执行插入收货地址模块,表名：shouhuodizhi
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                        ext.put("addtime", Info.getDateStr());

    String charuid = new CommDAO().insert(request,"shouhuodizhi",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    // 执行更新收货地址模块,表名：shouhuodizhi
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                                    
    String charuid = new CommDAO().update(request,"shouhuodizhi",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("shouhuodizhi").find(Request.get("id"));
    
    new CommDAO().delete(request,"shouhuodizhi");  // 执行删除命令
